...
function WishCard({ wish, onClick }: { wish: Wish; onClick: () => void }) {
  ...
  const floatDur = 5 + ((wish.created_at.charCodeAt(5) || 3) % 4)
  const floatDelay = (parseInt(wish.id.slice(-3), 16) % 30) / 10
  ...
  return (
    <button
      ...
      style={{
        position:'absolute', left:`${wish.pos_x}%`, top:`${wish.pos_y}%`,
        width:w, height:h,
    ...
    >
      <style>{`
        @keyframes floatCard {
          0% { transform: translate(-50%,-50%) rotate(-2.5deg) scale(1); }
          100% { transform: translate(-50%,-50%) rotate(2.5deg) scale(1.07); }
        }
      ...
      <div className="wcard">
        ...
        <div style={{ position:'absolute', inset:0, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding: wish.shape==='cloud' ? '1.3rem 1rem 0.3rem' : '0.8rem', textAlign:'center', pointerEvents:'none' }}>
          <span style={{ fontFamily:'Georgia, serif', fontWeight:'bold', fontSize:'0.73rem', color:'#2D5016', letterSpacing:'0.03em', marginBottom:'2px', lineHeight:1.2, maxWidth:'90%', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{wish.name}</span>
  ...
  return (
    <div onClick={onClose} style={{ position:'fixed', inset:0, background:'rgba(45,80,22,.32)', backdropFilter:'blur(5px)', zIndex:100, display:'flex', alignItems:'center', justifyContent:'center', padding:'1rem' }}>
      <div onClick={e => e.stopPropagation()} style={{ background:'var(--card-bg)', borderRadius:'24px', border:'2.5px solid var(--green)', boxShadow:'8px 8px 0 var(--green)', maxWidth:'480px', width:'100%', padding:'2rem', maxHeight:'88vh', overflowY:'auto', position:'relative', animation:'modalIn .28s ease' }}>
        ...
        <button onClick={onClose} style={{ position:'absolute', top:'1rem', right:'1rem', background:'none', border:'2px solid var(--green)', borderRadius:'50%', width:'30px', height:'30px', cursor:'pointer', color:'var(--green)', fontWeight:'bold', fontSize:'1rem', display:'flex', alignItems:'center', justifyContent:'center', lineHeight:1 }}>×</button>
  ...
  return (
    <div style={{ minHeight:'100vh', background:'var(--bg)', position:'relative', overflow:'hidden' }}>
      ...
      <header style={{ position:'sticky', top:0, zIndex:10, display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0.9rem 1.5rem', borderBottom:'2px solid #d4e9a0', background:'rgba(255,253,245,.95)', backdropFilter:'blur(8px)' }}>
        <Link href="/" style={{ textDecoration:'none', color:'var(--green)', fontFamily:'Helvetica', fontSize:'0.82rem', opacity:0.7, display:'flex', alignItems:'center', gap:'0.3rem' }}>← home</Link>
      ...
      <div style={{ position:'relative', width:'100%', height:'calc(100vh - 62px)', minHeight:'500px' }}>
        ...
        {!loading && !error && wishes.length === 0 && (
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', height:'100%', gap:'1rem', textAlign:'center', padding:'2rem' }}>
            ...
            <p style={{ fontFamily:'Helvetica', color:'var(--green-light)', fontSize:'0.9rem' }}>Be the very first one to wish Ndut a happy birthday 🎂</p>
            <Link href="/msg" style={{ textDecoration:'none', color:'white', background:'var(--green)', padding:'0.75rem 1.6rem', borderRadius:'999px', fontFamily:'Helvetica', fontWeight:'bold' }}>Send a wish ✨</Link>
          </div>
        )}
        ...
        {!loading && !error && wishes.length > 0 && (
          <p style={{ position:'absolute', bottom:'1rem', left:'50%', transform:'translateX(-50%)', fontFamily:'Helvetica', fontSize:'0.73rem', color:'var(--green-pale)', background:'rgba(255,253,245,.82)', padding:'0.3rem 1rem', borderRadius:'999px', whiteSpace:'nowrap', zIndex:5, backdropFilter:'blur(4px)' }}>
