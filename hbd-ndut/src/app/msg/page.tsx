...
import { randomShape, randomColor } from '@/lib/supabase'
...
export default function MsgPage() {
  ...
  const input = (hasError?: boolean): React.CSSProperties => ({
    ...
    fontSize:'0.95rem', color:'var(--green)', outline:'none',
    transition:'border-color .2s, box-shadow .2s',
    resize:'vertical',
  })
  const label: React.CSSProperties = {
    display:'block', fontFamily:'Georgia, serif', fontWeight:'bold',
    fontSize:'0.95rem', color:'var(--green)', marginBottom:'0.4rem',
  }
  const fieldWrap: React.CSSProperties = { marginBottom:'1.5rem' }
  const err: React.CSSProperties = { color:'#c0392b', fontFamily:'Helvetica', fontSize:'0.78rem', marginTop:'0.3rem' }
  ...
  if (status === 'done') return (
    <div style={{ minHeight:'100vh', background:'var(--bg)', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'2rem', textAlign:'center' }}>
      ...
      <div style={{ display:'flex', gap:'1rem', flexWrap:'wrap', justifyContent:'center', animation:'fadeUp .5s .65s ease both', opacity:0 }}>
        <Link href="/board" style={{ textDecoration:'none', color:'white', background:'var(--green)', padding:'0.75rem 1.6rem', borderRadius:'999px', fontFamily:'Helvetica', fontWeight:'bold', fontSize:'0.95rem' }}>
          See the board 💌
        </Link>
        <button
          onClick={() => { setName(''); setMessage(''); setMemory(''); setPhotoFiles([]); setPhotoPreviews([]); setStatus('idle') }}
          style={{ background:'none', border:'2px solid var(--green)', color:'var(--green)', padding:'0.75rem 1.6rem', borderRadius:'999px', fontFamily:'Helvetica', fontSize:'0.95rem', cursor:'pointer' }}
        >
          Add another wish
        </button>
      </div>
  ...
  return (
    <div style={{ minHeight:'100vh', background:'var(--bg)', paddingBottom:'3rem' }}>
      ...
      <div style={{ maxWidth:'520px', margin:'0 auto', padding:'2rem 1.25rem', animation:'fadeIn .5s ease' }}>
        ...
        {/* Name */}
        <div style={fieldWrap}>
          <label style={label}>Your Name <span style={{ color:'#c0392b' }}>*</span></label>
          <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Tia, Marco, Kiki…" style={input(!!errors.name)} />
          {errors.name && <p style={err}>⚠ {errors.name}</p>}
        </div>

        {/* Birthday wish */}
        <div style={fieldWrap}>
          <label style={label}>Birthday Wishes for Tfia <span style={{ color:'#c0392b' }}>*</span></label>
          <textarea value={message} onChange={e => setMessage(e.target.value)} placeholder="Write your heartfelt birthday message here…" rows={5} style={input(!!errors.message)} />
          {errors.message && <p style={err}>⚠ {errors.message}</p>}
        </div>

        {/* Memory */}
        <div style={fieldWrap}>
          <label style={label}>My Favorite Memory with Tfia <span style={{ fontFamily:'Helvetica', fontWeight:'normal', color:'var(--green-pale)', fontSize:'0.8rem' }}>(optional)</span></label>
          <textarea value={memory} onChange={e => setMemory(e.target.value)} placeholder="Share a memory that makes you smile…" rows={3} style={input()} />
        </div>

        {/* Photo upload */}
        <div style={fieldWrap}>
          <label style={label}>My Favorite Photo of/with Tfia <span style={{ fontFamily:'Helvetica', fontWeight:'normal', color:'var(--green-pale)', fontSize:'0.8rem' }}>(optional, up to 3)</span></label>
          ...
          {photoFiles.length < 3 && (
            ...
            >
              ...
              <p style={{ fontFamily:'Helvetica', fontSize:'0.75rem', color:'var(--green-pale)', marginTop:'0.2rem' }}>JPG, PNG, GIF — up to 3 photos total</p>
            </div>
          )}
        ...
        {status === 'error' && (
          <p style={{ ...err, marginBottom:'1rem', textAlign:'center', fontSize:'0.88rem' }}>⚠ Something went wrong. Please try again.</p>
        )}
